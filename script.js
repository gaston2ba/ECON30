...
Promise.all([
  fetch("data/uqi-by-alcaldia.json").then(response => response.json()),
...
function valuesForDecade(variableKey, decade = state.decade) {
  return state.data.alcaldias
    .map(alcaldia => alcaldia.values?.[decade]?.[variableKey])
...
function scoreAlcaldia(alcaldia, decade = state.decade) {
  const parts = selectedVariables()
    .map(variable => normalize(alcaldia.values?.[decade]?.[variable.key], variable, decade))
...
function computeScores(decade = state.decade) {
  ...
  state.data.alcaldias.forEach(alcaldia => {
    bySlug.set(matchSlug(alcaldia.name), { alcaldia, score: scoreAlcaldia(alcaldia, decade), selected: selectedVariables() });
  });
...
function renderMap() {
  ...
  const paths = state.boundaries.features.map(feature => {
    ...
    return `<path class="alcaldia-path" d="${geometryPath(feature.geometry, project)}" fill="${colorFor(record?.score)}" data-name="${name}" data-score="${scoreLabel}"></path>`;
  }).join("");
  ...
  document.querySelectorAll(".alcaldia-path").forEach(path => {
...
function showTooltip(event, name, score) {
  ...
  const lines = record?.selected.slice(0, 5).map(variable => {
    const value = record.alcaldia.values?.[state.decade]?.[variable.key];
...
function renderLineChart() {
  ...
  const lines = state.data.alcaldias.map((alcaldia, alcaldiaIndex) => {
    ...
    const dot = active ? `<circle class="chart-dot" cx="${active[0]}" cy="${active[1]}" r="4" fill="${lineColors[alcaldiaIndex % lineColors.length]}"></circle>` : "";
    return `<path class="chart-line" d="${d}" stroke="${lineColors[alcaldiaIndex % lineColors.length]}" data-name="${alcaldia.name}"></path>${dot}`;
  }).join("");
...
function updateMethodSummary() {
  ...
  document.getElementById("methodSummary").textContent = selected.length
    ? `${state.decade}: equal-weight UQI using ${selected.join(", ")}. Variables are min-max normalized across the 16 alcaldías within each decade.`
...
function updateRanking() {
  ...
  document.getElementById("rankingList").innerHTML = ranking
    .map(record => `<li>${record.alcaldia.name}: <strong>${Math.round(record.score * 100)}</strong></li>`)
    .join("");
}
